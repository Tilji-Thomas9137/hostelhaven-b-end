...
router.post('/save', authMiddleware, asyncHandler(async (req, res) => {
  ...
  // Start with a clean object and only add allowed fields
  ...
  // Add only the allowed user_profiles fields - EXPLICITLY exclude users fields
  ...
  // EXPLICITLY remove any users table fields that might have been added
  delete safeProfileData.full_name;
  delete safeProfileData.phone;
  delete safeProfileData.email;
  delete safeProfileData.password_hash;
